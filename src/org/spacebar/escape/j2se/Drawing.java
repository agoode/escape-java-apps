...
public class Drawing {
    ...
    private final static BufferedImage[][] bombs;
    ...
    static {
        ...
        bots = new BufferedImage[5][];
        bombs = new BufferedImage[5][];
        ...
        String bNames3[] = { "animation/dalek_asleep.png" };
        bots[3] = ResourceUtil.createScaledImages(ResourceUtil
                .stitchHoriz(bNames3), SCALE_DOWN_FACTORS, SCALE_UP_FACTORS);
        String bNames4[] = { "animation/hugbot_asleep.png" };
        bots[4] = ResourceUtil.createScaledImages(ResourceUtil
                .stitchHoriz(bNames4), SCALE_DOWN_FACTORS, SCALE_UP_FACTORS);

        // bomb
        String bNames5[] = { "animation/bomb_still.png" };
        bombs[0] = ResourceUtil.createScaledImages(ResourceUtil
                .stitchHoriz(bNames5), SCALE_DOWN_FACTORS, SCALE_UP_FACTORS);
        String bNames6[] = { "animation/bomb_lit0.png" };
        bombs[1] = ResourceUtil.createScaledImages(ResourceUtil
                .stitchHoriz(bNames6), SCALE_DOWN_FACTORS, SCALE_UP_FACTORS);
        String bNames7[] = { "animation/bomb_lit1.png" };
        bombs[2] = ResourceUtil.createScaledImages(ResourceUtil
                .stitchHoriz(bNames7), SCALE_DOWN_FACTORS, SCALE_UP_FACTORS);
        String bNames8[] = { "animation/bomb_lit2.png" };
        bombs[3] = ResourceUtil.createScaledImages(ResourceUtil
                .stitchHoriz(bNames8), SCALE_DOWN_FACTORS, SCALE_UP_FACTORS);
        String bNames9[] = { "animation/bomb_lit3.png" };
        bombs[4] = ResourceUtil.createScaledImages(ResourceUtil
                .stitchHoriz(bNames9), SCALE_DOWN_FACTORS, SCALE_UP_FACTORS);
    ...
    private static void paintBot(Graphics2D g2, Level theLevel, int xScroll,
        ...
        boolean isBomb = theLevel.isBotBomb(botIndex);
        if (isBomb) {
            imgs = bombs;
            botType = theLevel.getBombTimer(botIndex) + 1; // draw correct fuse
            if (botType >= imgs.length) {
                botType = imgs.length - 1;
            }
        } else {
            imgs = bots;
        }
        ...
        int height = imgs[botType][zoom].getHeight();
        ...
        g2.drawImage(imgs[botType][zoom], dx, dy, dx + tileSize, dy + height,
        ...
        if (isBomb) {
            if (botType != 0) {
                drawString(g2, Characters.YELLOW
                        + Integer.toString(theLevel.getBombTimer(botIndex)));
            }
        } else {
